...
WARNING: This link could potentially be dangerous`)) {
  ...
  var Flock = class {
    ...
    connect() {
      ...
      this.ws.onclose = () => setTimeout(() => this.connect(), 2e3);
    }
    handle(msg) {
      switch (msg.type) {
        case "created": {
    ...
    `;
      ...
      el.addEventListener("mousedown", () => {
        if (pane.sid) this.setActive(pane.sid);
      });
      el.querySelector(".close").addEventListener("click", (e) => {
        e.stopPropagation();
        if (pane.sid) {
          this.send("kill", pane.sid);
          this.removePane(pane.sid);
        }
      });
      ...
      term.onData((data) => {
        if (pane.sid) this.send("input", pane.sid, data);
      });
      ...
      document.addEventListener("keydown", (e) => {
        ...
        if (e.metaKey && !e.shiftKey && e.key === "w") {
          ...
          if (this.activePaneId) {
            this.send("kill", this.activePaneId);
            this.removePane(this.activePaneId);
          }
          return;
        }
